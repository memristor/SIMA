...
//  uint64_t t_start = esp_timer_get_time();
  ...
  // Ovde ce biti 0 svuda jer je bafer na PCB aktivan na 0 i oba en su kratko spojeni, a da se ne brise ipak toggle
...
//  printf(" esp_timer start: %llu end %llu delta %llu\n",t_start,t_end,t_end-t_start);
  ...
  ESP_LOG_BUFFER_HEX(TAG, packet, 16);
...
uint8_t setupPortEspIdf(int port_num)
{
  PortData *pd = &portData[port_num];
    ...
    ESP_ERROR_CHECK(uart_set_rx_timeout(uart_num, 2 /* timeout */)); // It was 0

    // Must be set after driver is installed
    ESP_ERROR_CHECK(uart_set_mode(uart_num, UART_MODE_UART)); // It was UART_MOED_RS485_HALF_DUPLEX
