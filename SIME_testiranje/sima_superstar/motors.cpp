...
void rotateMotors(double angle_deg) {
  const double WHEEL_DISTANCE_MM = 95;
  const double TICKS_PER_MM = 4095.0 / 265.485;
...
void resetMotors() {
  ...
  while (true) {
    if (!readPosition(pos)) continue;
    if (abs(pos[0] - goal_positions[0]) <= 20 && abs(pos[1] - goal_positions[1]) <= 20) break;
    delay(10);
  }
